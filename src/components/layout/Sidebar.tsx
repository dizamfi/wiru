    ...
    return (
      <Link
        ...
        className={cn(
          ...
          active
            ? 'bg-gradient-to-t from-[#99bb44] to-[#7fa836] text-white font-semibold' // ← CAMBIO: Gris suave + letras negras
      ...
      >
        <Icon
          className={cn(
            'mr-3 h-5 w-5 flex-shrink-0',
            active ? 'text-white' : 'text-gray-400 group-hover:text-gray-600' // ← CAMBIO: Iconos negros cuando activo
          )}
        />
        <span className="flex-1">{item.name}</span>
